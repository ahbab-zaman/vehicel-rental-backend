...
export class BookingService {
  ...
  static async updateBooking(bookingId: number, updateData: any, user: any) {
    const client = await pool.connect();
    ...
    try {
      ...
      if (user.role === "customer" && status === "cancelled") {
        ...
        // Cancel booking and make vehicle available
      ...
      if (user.role === "admin" && status === "returned") {
        ...
        // Mark as returned and make vehicle available
        await client.query("UPDATE bookings SET status = $1 WHERE id = $2", [
          "returned",
          bookingId,
        ]);
  ...
  static async autoReturnExpiredBookings() {
    const client = await pool.connect();
